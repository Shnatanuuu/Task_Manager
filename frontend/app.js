...
function setupUserInterface() {
    ...
    document.getElementById('user-department').textContent = state.user.department || 'Department';
    ...
    document.querySelector('.wfh-section').style.display = isAdmin ? 'none' : 'block';
    
    // Hide task creation for employees
    document.getElementById('add-task-btn').style.display = isEmployee ? 'none' : 'block';
    document.getElementById('assigned-by-me-option').style.display = isEmployee ? 'none' : 'block';
    
    // Show employee selector for HOD/Admin in calendar
    document.getElementById('employee-select').style.display = isEmployee ? 'none' : 'block';
}
...
async function loadKanbanData() {
    try {
        const [tasks, departmentUsers] = await Promise.all([
            apiRequest('GET', '/tasks'),
            state.user.role !== 'Employee' ? apiRequest('GET', `/departments/${state.user.departmentId}/users`) : Promise.resolve([])
        ]);
...
async function toggleAttendance() {
    try {
        const endpoint = state.attendanceStatus?.isCheckedIn ? '/attendance/checkout' : '/attendance/checkin';
...
function renderKanbanBoard() {
    ...
    // Update task counts
    document.querySelector('[data-status="To Do"] .task-count').textContent = tasksByStatus['To Do'].length;
    document.querySelector('[data-status="In Progress"] .task-count').textContent = tasksByStatus['In Progress'].length;
    document.querySelector('[data-status="Done"] .task-count').textContent = tasksByStatus['Done'].length;
...
function setupKanbanFilters() {
    document.getElementById('task-filter').addEventListener('change', renderKanbanBoard);
}
...
async function handleTaskMove(taskId, newStatus) {
    try {
        ...
        showToast('Update Failed', 'You don\'t have permission to move this task', 'error');
    }
}
            ...
            <div class="calendar-day ${isToday ? 'today' : ''} ${isSelected ? 'selected' : ''}" 
                ...
                <div class="day-tasks">
                    ${tasksForDay.slice(0, 1).map(task => `
                        <div class="task-preview ${task.priority.toLowerCase()}">${task.title}</div>
                    `).join('')}
                    ${logsForDay.slice(0, 2).map(log => `
                        <div class="task-preview log">✓ ${log.description.substring(0, 20)}${log.description.length > 20 ? '...' : ''}</div>
                    `).join('')}
                    ${(tasksForDay.length + logsForDay.length) > 3 ? `
                        <div class="task-overflow">+${(tasksForDay.length + logsForDay.length) - 3} more</div>
                    ` : ''}
                </div>
            </div>
        `;
...
function getTasksForDate(date) {
    return state.tasks.filter(task => {
        ...
        const isRelevantTask = state.user.role === 'Employee' 
...
function renderSelectedDateDetails() {
    ...
    if (!state.selectedDate) {
        ...
        contentElement.innerHTML = '<p class="no-data">Click on a date to view tasks and performance logs</p>';
        return;
    }
    ...
    content += `
        <div class="detail-section">
            ...
            `).join('') : '<p class="no-data">No assigned tasks</p>'}
        </div>
    `;
    ...
    content += `
        <div class="detail-section">
            ...
            ${logsForDate.length > 0 ? logsForDate.map(log => `
                <div class="log-detail">
                    <div class="log-description">${log.description}</div>
                    <div class="log-meta">
                        ${log.startTime && log.endTime ? `
                            <span>${new Date(log.startTime).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})} - 
                            ${new Date(log.endTime).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span>
                        ` : ''}
                        ${log.durationMinutes ? `
            ...
            `).join('') : '<p class="no-data">No tasks performed</p>'}
        </div>
    `;
...
function renderDateLogs() {
    ...
    if (logsForDate.length === 0) {
        container.innerHTML = '<p class="no-data">No tasks logged for this date</p>';
        return;
    }
    
    container.innerHTML = logsForDate.map(log => `
        <div class="log-item performed">
            <div class="log-description">${log.description}</div>
            <div class="log-meta">
                ${log.startTime && log.endTime ? `
                    <div class="log-time">
                        <span><i class="fas fa-clock"></i> ${new Date(log.startTime).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})} - 
                        ${new Date(log.endTime).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span>
                        ${log.durationMinutes ? `<span><i class="fas fa-stopwatch"></i> ${Math.floor(log.durationMinutes / 60)}h ${log.durationMinutes % 60}m</span>` : ''}
                    </div>
                ` : ''}
                <span><i class="fas fa-calendar"></i> ${new Date(log.createdAt).toLocaleDateString()}</span>
            </div>
        </div>
    `).join('');
}
...
function setupDateFilter() {
    document.getElementById('date-filter').addEventListener('change', renderDateLogs);
}
...
async function createTaskLog(logData) {
    try {
        ...
        if (logData.startTime && logData.endTime) {
            ...
            logData.durationMinutes = Math.round((end.getTime() - start.getTime()) / (1000 * 60));
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Attendance toggle
    document.getElementById('attendance-toggle').addEventListener('click', toggleAttendance);
    ...
    document.getElementById('task-log-form').addEventListener('submit', async (e) => {
        e.preventDefault();
        const formData = new FormData(e.target);
